...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, properties = {"spring.jersey.application-path=/api"})
class HelloResourceBasePathTest {
    ...
    void should_return_hello_message_from_jersey_endpoint() throws Exception {
        // Given
        var url = "http://localhost:%d/api/hello".formatted(port);
    ...
    void should_return_hello_with_name_from_jersey_endpoint() throws Exception {
        // Given
        var name = "John";
        var url = "http://localhost:%d/api/hello/%s".formatted(port, name);
        var request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .GET()
                .build();
